...
export class Home implements AfterViewInit {
  ...
  async onLogin() {
    try {
      ...
      if (userSnap.exists()) {
        ...
        switch (role) {
          case 'client': this.router.navigateByUrl('/pages/clientdashboard'); break;
          case 'consultant': this.router.navigateByUrl('/consultant/consultantdashboard'); break;
          case 'admin': this.router.navigateByUrl('/admin'); break;
          default: alert('Unknown role. Please contact support.');
        }
      } else {
        alert('User record not found in database.');
      }
  ...
  showForgotModal() {
    bootstrap.Modal.getOrCreateInstance(document.getElementById('signupModal')!).hide();
    bootstrap.Modal.getOrCreateInstance(document.getElementById('forgotModal')!).show();
  }
  ...
  async sendResetLink() {
    ...
    try {
      await sendPasswordResetEmail(this.auth, this.resetEmail);
      alert('Reset link sent! Please check your email.');
      bootstrap.Modal.getOrCreateInstance(document.getElementById('forgotModal')!).hide();
      this.resetEmail = '';
    } catch (err: any) {
      alert('Failed to send reset link: ' + err.message);
    }
